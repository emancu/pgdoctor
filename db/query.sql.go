...
type QueryStatisticsWindowRow struct {
	StatsReset pgtype.Timestamptz
	AgeSeconds pgtype.Int8
}
...
func (q *Queries) QueryStatisticsWindow(ctx context.Context) (QueryStatisticsWindowRow, error) {
	row := q.db.QueryRow(ctx, queryStatisticsWindow)
	var i QueryStatisticsWindowRow
	err := row.Scan(&i.StatsReset, &i.AgeSeconds)
	return i, err
}
