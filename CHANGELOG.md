...
- **`freeze-age`**: new `horizon-pin` finding answering "kill something or tune something" when the freeze age is high. Reads durable pins only — a replication slot's `xmin`/`catalog_xmin` and prepared transactions, on-disk state that never resolves itself — so backends and lock waiters stay out (those need a live command). WARN when a pin reaches `1 × autovacuum_freeze_max_age`, when the database age reaches `2 × trigger` with a pin level with it, or when an inactive slot with reserved WAL holds a pin past 1M transactions; FAIL at `min(4 × trigger, vacuum_failsafe_age)` on either age, and at any age for an inactive slot whose WAL is `unreserved`/`lost`. Remediation is single-object, and dropping a logical slot is flagged as a re-snapshot with product impact rather than a DBA action ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: new `database-multixact-age` and `table-multixact-age` findings covering the MultiXact wraparound counter — an independent clock with its own trigger (`autovacuum_multixact_freeze_max_age`) that RDS exposes no CloudWatch metric for. Thresholds use the same derivation as the XID findings against the counter's own GUCs: WARN at `2 × effective autovacuum_multixact_freeze_max_age` (800M at defaults) and FAIL at `min(4 × effective trigger, vacuum_multixact_failsafe_age)` (1.6B), honouring a per-table `autovacuum_multixact_freeze_max_age` reloption ([#60](https://github.com/emancu/pgdoctor/pull/60)).
...
- **`freeze-age`**: thresholds are now derived per relation and database from the effective anti-wraparound trigger (`autovacuum_freeze_max_age`, honouring per-table `reloptions`, which can only lower it) instead of hardcoded 500M/1B/400M/800M: WARN at `2 × trigger` (400M at defaults) and FAIL at `min(4 × trigger, vacuum_failsafe_age)` (800M). A relation's age is a sawtooth whose peak IS the trigger — nothing vacuums a low-churn relation until the trigger fires — so an age just below it is the healthy top of the cycle, and only exceeding it means autovacuum is losing ground. FAIL anchors on `vacuum_failsafe_age` (1.6B) rather than the 2B wall, whose documented single-user recovery does not exist on RDS ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: `table-freeze-age` now reports one row per VACUUM target rather than per relation — TOAST relations are folded into the parent they are vacuumed through, so `pg_toast_NNNNN` rows no longer fill the table — and expresses age as a multiple of the trigger (`1.0×` normal, `2.0×` WARN, `4.0×` FAIL) rather than a percentage, which read 99-100% on every healthy relation. Reports the total above the reporting floor when the list is truncated ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: relation coverage widened to `relkind IN ('r','m','t')` across all schemas (was `'r'` in `public` only), so TOAST relations, matviews, `pg_catalog` and orphaned `pg_temp_*` relations are included — this is what makes an abandoned logical slot's `catalog_xmin` pin visible. TOAST findings resolve to the parent table as the vacuum target ([#60](https://github.com/emancu/pgdoctor/pull/60)).
...
- **`freeze-age`**: `database-freeze-age` now reports the connected database only. It previously listed every `pg_database` row, so `template0`, `template1` and `postgres` produced findings that cannot be acted on from the current connection ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**, **`table-vacuum-health`**: relation size is now a lock-free `relpages`-derived estimate. `pg_total_relation_size()` takes an `AccessShareLock`, and a new AccessShareLock request queues behind a *waiting* `AccessExclusiveLock`, so both checks exceeded the statement timeout and SKIPped during exactly the DDL pile-up they exist to diagnose. Sizes on never-vacuumed relations render as `unknown` rather than `0 B` ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`connection-health`**: README remediation no longer suggests set-valued `pg_terminate_backend(...)` over `pg_stat_activity` — a mass kill against a live primary — and the lock/blocked-queries branch of the decision tree now points at `houston dba xmin` instead of `idle-in-transaction`, which does not detect xmin-horizon pins ([#60](https://github.com/emancu/pgdoctor/pull/60)).
