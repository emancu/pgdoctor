...
- **`freeze-age`**: new `horizon-blockers` finding naming what pins the xmin horizon and prevents vacuum advancing `relfrozenxid` — replication slot `xmin`/`catalog_xmin`, prepared transactions, and long-lived backend pins gated on XID age. Reports when the oldest pin is too young to explain the horizon, so the operator stops hunting for a PID and looks at vacuum throughput instead.
- **`freeze-age`**: new `doom-loop` finding — a pin at or beyond the effective trigger guarantees the next anti-wraparound vacuum completes without advancing `relfrozenxid` and is re-queued within `autovacuum_naptime`, non-cancellable, indefinitely.
...
- **`connection-health`**: README remediation no longer suggests set-valued `pg_terminate_backend(...)` over `pg_stat_activity` — a mass kill against a live primary — and the lock/blocked-queries branch of the decision tree now points at `freeze-age`'s `horizon-blockers` instead of `idle-in-transaction`, which does not detect horizon pins.
