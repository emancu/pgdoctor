...
- **`freeze-age`**: new `database-multixact-age` and `table-multixact-age` findings covering the MultiXact wraparound counter — an independent clock with its own trigger (`autovacuum_multixact_freeze_max_age`) that RDS exposes no CloudWatch metric for. Thresholds use the same derivation as the XID findings, against the counter's own GUCs: WARN at `min(vacuum_multixact_freeze_table_age, 0.95 × effective trigger)` (150M at defaults) and FAIL at `min(4 × effective trigger, vacuum_multixact_failsafe_age)` (1.6B), honouring a per-table `autovacuum_multixact_freeze_max_age` reloption ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: new `horizon-blockers` finding naming what pins the xmin horizon and prevents vacuum advancing `relfrozenxid` — replication slot `xmin`/`catalog_xmin`, prepared transactions, and long-lived backend pins gated on XID age. Reports when the oldest pin is too young to explain the horizon, so the operator stops hunting for a PID and looks at vacuum throughput instead ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: new `doom-loop` finding — a pin at or beyond the effective trigger guarantees the next anti-wraparound vacuum completes without advancing `relfrozenxid` and is re-queued within `autovacuum_naptime`, non-cancellable, indefinitely ([#60](https://github.com/emancu/pgdoctor/pull/60)).
...
- **`freeze-age`**: thresholds are now derived per relation and database from `autovacuum_freeze_max_age`, `vacuum_freeze_table_age` and `vacuum_failsafe_age` — honouring per-table `reloptions`, which can only lower the trigger — instead of hardcoded 500M/1B/400M/800M. WARN now fires at PostgreSQL's own aggressive-scan point (150M at defaults) rather than long after anti-wraparound vacuum started losing, and FAIL anchors on `vacuum_failsafe_age` (1.6B) rather than the 2B wall, whose documented single-user recovery does not exist on RDS ([#60](https://github.com/emancu/pgdoctor/pull/60)).
- **`freeze-age`**: `table-freeze-age` now leads with remaining XID headroom to the trigger rather than raw age, ranked by fraction of the trigger consumed, and reports the total above the reporting floor when the list is truncated ([#60](https://github.com/emancu/pgdoctor/pull/60)).
...
- **`freeze-age`**: dropped the `datallowconn = true` filter. The cluster-wide XID limit is `min(datfrozenxid)` over *every* `pg_database` row — `vac_truncate_clog()` does not filter on `datallowconn` — so `template0` and RDS's `rdsadmin` could trip the `MaximumUsedTransactionIDs` alarm while the check reported PASS. Those databases are now reported and labelled as not fixable by a plain `VACUUM` ([#60](https://github.com/emancu/pgdoctor/pull/60)).
...
- **`connection-health`**: README remediation no longer suggests set-valued `pg_terminate_backend(...)` over `pg_stat_activity` — a mass kill against a live primary — and the lock/blocked-queries branch of the decision tree now points at `freeze-age`'s `horizon-blockers` instead of `idle-in-transaction`, which does not detect horizon pins ([#60](https://github.com/emancu/pgdoctor/pull/60)).
