...
- **`freeze-age`**: new `database-multixact-age` and `table-multixact-age` findings covering the MultiXact wraparound counter — an independent clock with its own trigger (`autovacuum_multixact_freeze_max_age`) that RDS exposes no CloudWatch metric for.
