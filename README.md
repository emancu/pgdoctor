...
When the DSN sets no `connect_timeout`, pgdoctor uses a connect timeout of 10 seconds. When the DSN sets no `application_name`, pgdoctor uses `pgdoctor`, so you can find its session in `pg_stat_activity`. A value in the DSN always wins.
